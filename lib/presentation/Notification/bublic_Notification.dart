...
import 'Tablet/bublic_Notification_tablet.dart';

class FcmPublic extends StatefulWidget {
  const FcmPublic({super.key});

  @override
  State<FcmPublic> createState() => _FcmPublicState();
}

class _FcmPublicState extends State<FcmPublic> {
  todayDate() {
    ...
    return Scaffold(
      body: LayoutBuilder(builder: (context, constrain) {
        if (constrain.maxWidth < 600) {
                      ...
                      GestureDetector(
                        onTap: () async {
                          if (keyFCMPublic.currentState!.validate()) {
                            ...
                            await PublicNotificationCubit.get(context)
                                .seFcm(
                                    fcm: bodyController.text, date: todayDate())
                                .then((value) async {
                              await clear();
          ...
          return const Center(child: FcmPublicTablet());
        } else {
